...
namespace iayos.DapperCRUD.Test
{
	#region DTOClasses
	//For .Net 4.5> [System.ComponentModel.DataAnnotations.Schema.Table("Users")]  or the attribute built into DapperCRUD
	[Table("Users")]
	public class User
	{
	...
	public class Tests
	{
		public Tests(DapperCRUD.Dialect dbtype)
		{
			_dbtype = dbtype;
		}
		private DapperCRUD.Dialect _dbtype;

		private IDbConnection GetOpenConnection()
		{

			IDbConnection connection;
			if (_dbtype == DapperCRUD.Dialect.PostgreSQL)
			{
				...
				DapperCRUD.SetDialect(DapperCRUD.Dialect.PostgreSQL);
			}
			else if (_dbtype == DapperCRUD.Dialect.SQLite)
			{
				connection = new SQLiteConnection("Data Source=MyDatabase.sqlite;Version=3;");
				DapperCRUD.SetDialect(DapperCRUD.Dialect.SQLite);
			}
			else if (_dbtype == DapperCRUD.Dialect.MySQL)
			{
				...
				DapperCRUD.SetDialect(DapperCRUD.Dialect.MySQL);
			}
			else
			{
				connection = new SqlConnection(@"Data Source = (LocalDB)\v11.0;Initial Catalog=DapperDapperCRUDTestDb;Integrated Security=True;MultipleActiveResultSets=true;");
				DapperCRUD.SetDialect(DapperCRUD.Dialect.SQLServer);
			}
		...
		{
			DapperCRUD.SetDialect(DapperCRUD.Dialect.SQLServer);
			DapperCRUD.GetDialect().IsEqualTo(DapperCRUD.Dialect.SQLServer.ToString());
			DapperCRUD.SetDialect(DapperCRUD.Dialect.PostgreSQL);
			DapperCRUD.GetDialect().IsEqualTo(DapperCRUD.Dialect.PostgreSQL.ToString());
		}
