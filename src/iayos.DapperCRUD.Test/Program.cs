	...
	class Program
	{
		...
		private static void Setup()
		{
			using (var connection = new SqlConnection(@"Data Source=(LocalDB)\v11.0;Initial Catalog=Master;Integrated Security=True"))
			{
				...
				try
				{
					connection.Execute(@" DROP DATABASE DapperDapperCRUDTestDb; ");
				}
				catch (Exception)
				{ }

				connection.Execute(@" CREATE DATABASE DapperDapperCRUDTestDb; ");
			}

			using (var connection = new SqlConnection(@"Data Source = (LocalDB)\v11.0;Initial Catalog=DapperDapperCRUDTestDb;Integrated Security=True"))
			{
				connection.Open();
		...
		private static void RunTests()
		{
			...
			var sqltester = new Tests(DapperCRUD.Dialect.SQLServer);
			...
			using (var connection = new SqlConnection(@"Data Source=(LocalDB)\v11.0;Initial Catalog=Master;Integrated Security=True"))
			{
				...
				try
				{
					...
					connection.Execute(@" alter database DapperDapperCRUDTestDb set single_user with rollback immediate; DROP DATABASE DapperDapperCRUDTestDb; ");
				}
				catch (Exception)
				{ }
			}
		...
		{
			var stopwatch = Stopwatch.StartNew();
			var pgtester = new Tests(DapperCRUD.Dialect.PostgreSQL);
			foreach (var method in typeof(Tests).GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly))
			{
				var testwatch = Stopwatch.StartNew();
		...
		{
			var stopwatch = Stopwatch.StartNew();
			var pgtester = new Tests(DapperCRUD.Dialect.SQLite);
			foreach (var method in typeof(Tests).GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly))
			{
				//skip schema tests
		...
		{
			var stopwatch = Stopwatch.StartNew();
			var mysqltester = new Tests(DapperCRUD.Dialect.MySQL);
			foreach (var method in typeof(Tests).GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly))
			{
				//skip schema tests
				if (method.Name.Contains("Schema")) continue;
